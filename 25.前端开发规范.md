...
获取选择选中状态，使用dom节点对象的checked属性，或者jQuery是is方法，不要使用attr('checked')进行判断。
...
### 3.3. 前端开发的JavaScript技巧

#### 3.3.1 使用Angular是，ajax请求请使用ng自带的$http,避免使用jQuery的ajax
...
#### 3.3.2 HTTP请求中，获取使用GET，提交、修改和删除使用POST

这样更加符合REST的语义，如果GET使用POST代替，在开发阶段就难以调试，而且无法使用GET的特性进行一系列调优，例如缓存等。

HTTP中，GET/POST/DEL等的区别：http://www.cnblogs.com/zhangpengshou/archive/2012/07/09/2583096.html

#### 3.3.3 获取ajax的表单参数，jQuery可以将form表单序列化，Angualr可以绑定对象
